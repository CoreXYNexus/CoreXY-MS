...
module XMountWCSingle(Screw=Yes4mmInsert(Use4mmInsert),DoTabSupport=1) {
	difference() {
		union() {
			color("red") hull() {
				translate([44,42,35/2]) color("red") cuboid([20,Thickness+2,35],rounding=2); // wirechain mount
				translate([2.5,42,10]) color("red") cuboid([5,Thickness+2,20],rounding=2); // wirechain mount
			}
			color("cyan") cuboid([Thickness+1,45,20],rounding=2,p1=[0,0]); // mount to xcarriage
		}
		translate([43,50,15]) rotate([90,0,0]) WCSingleHoles(Screw);
		translate([2,11,10]) rotate([0,90,0]) XMountHoles(screw5);
		translate([20,55,21]) color("blue") rotate([90,0,0]) cylinder(h=20,d=screw5); // ziptie hole
	}
	if(DoTabSupport) {
		translate([4,3,0]) color("green") cylinder(h=LayerThickness,d=20); // print support TabSupport
		translate([52,43,0]) color("gray") cylinder(h=LayerThickness,d=20); // print support TabSupport
	}
}
...
module YAxisWireChain(Holes=1) {	// 	// on frame for y axis
	difference() { // wireguide mount
		union() {
			if(Holes) color("black") cuboid([Thickness+2,Width,TabSupport],rounding=2,p1=[0,0]);
			SupportBrace();
	...
	if(!Holes) color("lightgray") cuboid([Thickness,Width,TabSupport],rounding=2,p1=[0,0]);
}
...
module SupportBrace() {
	difference() {
		translate([-15,Width/2-2.25,7.5]) rotate([0,40,0]) color("pink") cuboid([Width,Thickness,TabSupport],rounding=2,p1=[0,0]);
