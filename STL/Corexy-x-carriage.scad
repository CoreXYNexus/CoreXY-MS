...
module partial() {
	//carriage(0,0);	// makerslide x-carriage, set arg to 1 for a titan thumbwheel notch,
	...
	//translate([0,-45,26]) rotate([-90,0,0]) // this puts the titan mount on the carraige
		titan(4);	// extruder platform for e3d titan with (0,1)BLTouch or (2)Proximity or (3)dc42's ir sensor
...
module carriage(Titan=0,Tshift=0) { // wheel side
	difference() {
		...
		if(Titan) translate([-20-Tshift,10,2]) cylinder(h=5,d=20,$fn=100); // Titan thumbwheel notch
...
module titan(recess=3) { // extruder platform for e3d titan with (0,1)BLTouch or (2)Proximity or (3)dc42's ir sensor
	...
	if(recess == 4) {
		translate([30,0,-wall/2]) blt_mount(1,shiftblt); // BLTouch mount
		translate([30,-35,-wall/2]) prox_mount(shiftprox);
		translate([75,-10,-wall/2]) {
			difference() {
				iradapter(0,ir_height);
				translate([25,4,40]) rotate([90,0,0]) ir_mount_screws(screw3);
			}
		}
	}
}
...
module titan3(Screw=screw4) { // extruder platform for e3d titan to mount directly on the x-carraige
	difference() {
		translate([0,0,0]) color("indianred") cubeX([40,56,5],2); // extruder side
		translate([20,28,-10]) cylinder(h=20,d=20,$fn=100); // remove some plastic under the motor
	}
...
module extmount() {		// screw holes to mount extruder plate
	...
	translate([width/2-5,30-wall/2,-10]) color("white") cylinder(h = 25, r = screw3/2, $fn = 50);
	translate([-(width/2-5),30-wall/2,-10]) color("blue") cylinder(h = 25, r = screw3/2, $fn = 50);
	translate([width/4-2,30-wall/2,-10]) color("green") cylinder(h = 25, r = screw3/2, $fn = 50);
	translate([-(width/4-2),30-wall/2,-10]) color("purple") cylinder(h = 25, r = screw3/2, $fn = 50);
}
...
module titanmotor(WallMount,Screw=screw4) {
	difference() {	// motor mounting holes
		if(WallMount) {
			translate([-1,0,0]) color("red") cubeX([57,50+shifttitanup,5],2);
		} else {
			translate([-1,0,0]) color("red") cubeX([54,50+shifttitanup,5],2);
		}
		translate([25,25+shifttitanup,-1]) rotate([0,0,45]) color("purple") NEMA17_x_holes(8, 2);
	}
	difference() { // front support
		translate([-1,0,0]) color("blue") cubeX([4,50,50],2);
		translate([-3,50,4]) rotate([56,0,0]) cube([7,50,70]);
		translate([-4,-4,36]) cube([wall,wall,wall]);
	}
	if(WallMount) {
		difference() { // rear support
			translate([52,0,0]) color("cyan") cubeX([4,50+shifttitanup,40],2);
			translate([40,15,15]) rotate([0,90,0]) cylinder(h=20,d=Screw,$fn=100); // mounting screw hole
			translate([40,15+i3x_sep,15]) rotate([0,90,0])  cylinder(h=20,d=Screw,$fn=100); // mounting screw hole
			if(Screw==screw5) { // add screw holes for horizontal extrusion
				translate([40,15,30]) rotate([0,90,0]) cylinder(h=20,d=Screw,$fn=100); // mounting screw hole
				translate([40,15+i3x_sep,30]) rotate([0,90,0])  cylinder(h=20,d=Screw,$fn=100); // mounting screw hole
			}
		}
	} else {
		difference() { // rear support
