...
//ExtruderPlateMount(0,0); 	// 1st arg: 0 - old style haotend mount, 1 - drill guide for old style, 2 - titan/e3dv6 mount
							//			3 - combo of 2 and x-carriage
							// 2nd arg: 0 | 1 bltouch, 2 - round proxmity, 3 - dc42's ir sensor
//TitanCarriage(0,2); // 1st arg: 1-one piece titan/e3dv6 on x-carriage + belt drive holder
...
module ProximityMount(Shift) {
	...
	difference() {
		...
		//translate([-16,60,53+Shift]) rotate([90,0,90]) FanMountHoles(screw3);
		translate([26.5,60,7+Shift]) rotate([0,0,0]) IRMountHoles(screw3);
		translate([27.5,60,7+Shift]) rotate([0,0,0]) IRMountHoles(screw3);
	}
}
