...
module mount() {
	...
	difference() {
		translate([2,0,Switch_thk-4]) color("gray") cubeX([20,width,Switch_thk2],2);
		translate([10,6.5,1]) color("black") cylinder(h=Switch_thk*3,r=screw5/2);
		translate([10,26.5,1]) color("red") cylinder(h=Switch_thk*3,r=screw5/2);
	}
}
...
module clamp() {	// something to clamp the wires on the corexy belt holder
	difference() {
		color("plum") cubeX([15,width,4],2);
		translate([7.5,6.5,-1]) color("white") cylinder(h=Switch_thk*2,r=screw5/2);
		translate([7.5,26.5,-1]) color("pink") cylinder(h=Switch_thk*2,r=screw5/2);
	}
}
