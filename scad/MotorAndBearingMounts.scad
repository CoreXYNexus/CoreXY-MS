...
// 8/8/20	- Changed the supports for the bearing_support()
/////////////////////////////////////////////////////////////////////////////////////////
...
module all() {
	if($preview) %translate([20,10,-4]) cube([200,200,2],center=true); // show the 200x200 bed
	translate([0,-5,0]) motor_mount(1);
	translate([35,60,-2.5]) rotate([0,0,0]) bearing_bracket(0,"Right");
	translate([80,-30,F625ZDoulbleStack*2+b_height+40]) rotate([0,180,0]) bearing_support(2,0);
	translate([0,65,0]) motor_mount(0);
	translate([75,20,-2.5]) rotate([0,0,0]) mirror([1,0,0]) bearing_bracket(0,"Left"); // mirror it for the other side
}
...
//	translate([90,0,0]) mirror([1,0,0]) bearing_bracket(0,"Left"); // mirror it for the other side
	rotate([0,-90,0]) BearingSupport(2,0);  // arg is to added a makerslide notch
}
...
module bearing_bracket(TapIt=0,Label="") {
	...
	if(!TapIt) translate([b_posY-4,b_posX-4.5,b_height+30-8]) color("salmon") cube([10,10,LayerThickness]); // nut hole support
...
module BearingSupport(Qty=1,NotchIt=0) {	// keep the idler bearings from tilting on the bracket
	...
	for(x = [0 : Qty-1]) {
		translate([0,x*25,0]) {
			...
			translate([1,3.5,4]) color("green") cube([LayerThickness,screw5hd,32]);
			if(NotchIt) {
